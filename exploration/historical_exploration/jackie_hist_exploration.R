...
colnames(host_count_1511) <- c("id", "host_count")
...
colnames(host_count_151120) <- c("id", "host_count")


#create a table of the host counts before and after purge
purge_host_count <- inner_join(host_count_1511, host_count_151120, by = "id")
#rename columns
colnames(purge_host_count) <- c("id", "count_prepurge", "count_postpurge")
#add column to find the change
purge_host_count$change <- purge_host_count$count_postpurge - purge_host_count$count_prepurge
...
ggplot(purge_host_count_clean, aes(x = count_prepurge, y = count_postpurge)) + geom_point() + geom_abline(col = "red")

#look at those that were not in the pre and post
pre_post_host_count <- anti_join(host_count_1511, host_count_151120, by = "id")

#filter for just those that occur 3 times
purge_host_count_clean_3 <- purge_host_count_clean %>% filter(count_prepurge == 3)
View(purge_host_count_clean_3)

#find the number of listings that increased by 1, 0 change, and decreased by 1
purge_3_count <- purge_host_count_clean_3 %>% group_by(change) %>% summarize(total = n()) 
...
#create new table of just those that have host count of three to isolate this group
host_count_three <- prepurge %>% filter(host_count == 3)

#join that with host count clean 3 to find change
inner_join(host_count_three, purge_host_count_clean_3, by = "id") %>% View
...
prepurge_regression <- prepurge %>% select(host_since, host_response_time, host_response_rate, host_acceptance_rate, 
host_is_superhost, neighbourhood_cleansed, neighbourhood_group_cleansed, is_location_exact, property_type, accommodates, 
bathrooms, bedrooms, beds, bed_type, price, guests_included, extra_people, minimum_nights, maximum_nights, availability_30, 
availability_60, availability_90, availability_365, number_of_reviews, first_review, last_review, review_scores_rating, 
...
review_scores_value, instant_bookable, reviews_per_month, cancellation_policy, require_guest_profile_picture, 
...
#creates model
ridge.mod=glmnet(prepurge_matrix[train,], purge_matrix[train], family = "binomial", alpha = 0)
#predict on the test data
ridge.pred=predict(ridge.mod, s=4, newx = prepurge_matrix[test,])
set.seed(1)
cv.out=cv.glmnet(prepurge_matrix[train,], purge_matrix[train], alpha=0)
...
ridge.pred=predict(ridge.mod, s= bestlam, newx=prepurge_matrix[test,], type = "response")
#find the MSE on the test set
mean((ridge.pred-purge_matrix.test)^2)
#0.4612706
...
#lasso regression
#model
lasso.mod = glmnet(prepurge_matrix[train,], purge_matrix[train], family = "binomial", alpha = 1)
