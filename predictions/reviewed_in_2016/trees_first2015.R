...
jan_2015 <- skimmed %>% 
  filter(host_since >= "2015-01-01" & host_since <= "2015-12-31")

set.seed(321)


indexes <- sample(1:nrow(jan_2015), 
                  size=0.2*nrow(jan_2015))

#apply the indexes to listings history to generate test and train
test=jan_2015[indexes, ]
train=jan_2015[-indexes, ]
...
#save the tree file
jpeg(filename = "../../figures/all_features_reviews2016_2015start.jpeg")
rpart.plot(tree_pruned_all, left = TRUE)
dev.off()
...
jpeg(filename = "../../figures/best_features_reviews2016_2015start.jpeg")
...
jpeg(filename = "../../figures/rf_reviews2016_2015start.jpeg")
rpart.plot(tree_pruned_best, left = TRUE)
dev.off()
