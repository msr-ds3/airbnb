...
cv_listings <- 
  function(feature) {
    num_folds <- 10
    listings_auc <- nrow(skimmed)
    skimmed$fold <- sample(1:num_folds, listings_auc, replace=T)
    auc <- c()
    for (i in 1:num_folds) {
      set.seed(i)
      # use this data frame to fit your model
      listings_train <- filter(skimmed, fold != i)
      
      # pretend this doesn't exist (for now!)
      listings_test <- filter(skimmed, fold == i)
...
########################all features#########################################
tree_all <- (has_reviews_2016 ~ #property_type + room_type + mean_price + 
...
indexes <- sample(1:nrow(skimmed), 
                  size=0.2*nrow(skimmed))

#apply the indexes to listings history to generate test and train
test=skimmed[indexes, ]
train=skimmed[-indexes, ]
...
#save the tree file
jpeg(filename = "../../figures/all_features_reviews2016_nocohort.jpeg")
...
jpeg(filename = "../../figures/best_features_reviews2016_nocohort.jpeg")
...
jpeg(filename = "../../figures/rf_reviews2016_nocohort.jpeg")
rpart.plot(tree_pruned_best, left = TRUE)
dev.off()
