...
for (i in trees) {
  ...
  ROCR_predict <- prediction(predicted, test_all$exist_in_2016)
...
for (i in trees) {
  ...
  ROCR_predict <- prediction(predicted, test_part$exist_in_2016)
  roc.perf <- performance(ROCR_predict, measure = "tpr", x.measure = "fpr")
  auc_pred <- performance(ROCR_predict, measure = "auc")
  print(auc_pred@y.values)
}
