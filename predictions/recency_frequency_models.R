...
listings_history <- read_csv("../airbnb/raw_data/listing_history.csv", na='\\N')
...
indexes <- sample(1:nrow(listings_history), size=0.2*nrow(listings_history))
listings_history_test=listings_history[indexes, ]
listings_history_train=listings_history[-indexes, ]


#to predict number of reviews
model1 <- lm((log(num_reviews_2016 + 1)) ~ is_superhost_2015 + last_seen_month + 
               last_seen_review + total_occ_2015, data = listings_history_train)

#getting error message : In log(num_reviews_2016 + 1) : NaNs produced

which(is.nan(log(listings_history$num_reviews_2016 + 1)))

  # 1252 41759 47903
 #these values are -2, -2, -3

#filtered out listings that have negative number of reviews

listings_history_review <- listings_history %>% filter(num_reviews_2016 >= 0)
